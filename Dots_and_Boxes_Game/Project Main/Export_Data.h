/*
- Module Description:
= Defining Export_Data_Beginner() Function
= Exporting "Saving" The Game Data {Borders, Boxes, For Each Player}
= Arguments:
    $ Beginner_Board[6][6]
    $ Beginner_BoarD_Colour[6][6]
    $ Integer "To indicate number of file User had choosed to export data to".
    $ Array temptemp[9] "Contain all values We will need when Export and Import Data "
= >>>> Data Model <<<<
Line Number ]  Text
...
12          ]  50 51 52 53 54 55
            ...
            N N N N N N N N N[Borders - Boxes - FP Borders - FP Boxes - SP Borders - SP Boxes - FP turn - SP Turn - Board_Number]

=Defining Import_Data_Beginner() Function
=Importing "Reading" The Game Data {Borders, Boxes, For Each Player}
=Arguments:
     $ Beginner_Board[6][6]
     $ Beginner_BoarD_Colour[6][6]
     $ Integer "To indicate number of file User had choosed to Import data from it".
     $ Array temptemp[9] "Contain all values We will need when Export and Import Data "
The same Data Model as mentioned before.

-"Doing the same Functions for Advanced Mode by chaning the dimensions of Array from 6 to 12".
*/
...
void Export_Data_Advanced(int Advanced_Board[12][12],int Advanced_Board_Colour[12][12],int u,int temptemp[9])
{
    FILE *fp;
    switch(u)
    {
    ...
    // {
        for(int i=0 ; i<12 ; i++)
        {
            for(int j=0 ; j<12 ; j++)
            {
                fprintf(fp,"%d ",Advanced_Board[i][j]);
            }
            fprintf(fp,"\n");
        }
    // }
    ...
    // {
        for(int i=0 ; i<12 ; i++)
        {
            for(int j=0 ; j<12 ; j++)
            {
                fprintf(fp,"%d ",Advanced_Board_Colour[i][j]);
            }
            fprintf(fp,"\n");
        }
    // }
...
void Import_Data_Advanced(int Advanced_Board[12][12],int Advanced_Board_Colour[12][12],int u,int temptemp[9])
{
    FILE *fp;
    switch(u)
    {
    case 1: fp = fopen("PvP_A_G1.txt","r");break;
    case 2: fp = fopen("PvP_A_G2.txt","r");break;
    case 3: fp = fopen("PvP_A_G3.txt","r");break;
    };
    // for(int k=0 ; k<61 ; k++)
    // {
        for(int i=0 ; i<12 ; i++)
        {
            for(int j=0 ; j<12 ; j++)
            {
                fscanf(fp,"%d ",&Advanced_Board[i][j]);
            }
        }
    // }
    // for(int k=0 ; k<61 ; k++)
    // {
        for(int i=0 ; i<12 ; i++)
        {
            for(int j=0 ; j<12 ; j++)
            {
                fscanf(fp,"%d ",&Advanced_Board_Colour[i][j]);
            }
        }
    // }
    for(int i=0 ; i<9 ; i++)
